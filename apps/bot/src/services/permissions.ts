...
import { RESPONSE_LANGUAGE_PRIORITY, type SupportedLanguage } from "../constants/languages.js";
...
export function isAdmin(member: GuildMember): boolean {
  ...
  return member.roles.cache.some((role: Role) => ADMIN_ROLE_NAMES.has(role.name));
}

export function responseLanguageFor(member: GuildMember): SupportedLanguage {
...
function languageNameRole(language: SupportedLanguage): string {
  switch (language) {
    case "ru":
      return "russian";
    case "tr":
