...
export const groupCommand: BotCommand = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "create") return createGroup(interaction);
    if (subcommand === "add-channel") return addChannel(interaction);
    if (subcommand === "remove-channel") return removeChannel(interaction);
    if (subcommand === "list") return listGroups(interaction);
    if (subcommand === "channel-groups") return listChannelGroups(interaction);
  }
};

async function createGroup(interaction: ChatInputCommandInteraction): Promise<void> {
  const guild = interaction.guild;
  if (!guild) return;

  const name = interaction.options.getString("group-name", true).trim();
  const channel1 = interaction.options.getChannel("channel-1", true) as GuildTextBasedChannel;
  const channel2 = interaction.options.getChannel("channel-2", true) as GuildTextBasedChannel;
  const language1 = interaction.options.getString("language-1", true) as Language;
  const language2 = interaction.options.getString("language-2", true) as Language;
...
async function listGroups(interaction: ChatInputCommandInteraction): Promise<void> {
  const guild = interaction.guild;
  if (!guild) return;
...
async function listChannelGroups(interaction: ChatInputCommandInteraction): Promise<void> {
  const guild = interaction.guild;
  if (!guild) return;
...
async function ensureWebhookForGuildChannel(channel: GuildTextBasedChannel): Promise<void> {
