...
import { languageEmoji, languageName, parseLanguageCsv } from "../constants/languages.js";
...
export const rolesCommand = {
    ...
    async execute(interaction) {
        ...
        if (subcommand === "create")
            return createReactionRoles(interaction);
        if (subcommand === "assign")
            return assignRole(interaction);
        if (subcommand === "add-language")
            return addLanguage(interaction);
        if (subcommand === "remove-language")
            return removeLanguage(interaction);
        if (subcommand === "delete")
            return deleteReactionRoles(interaction);
    }
};
export function pendingRoleMessage(languages) {
    const lines = {
...
export function readyRoleMessage(languages) {
    const lines = {
...
export function reactionRoleMessagePayload(languages, complete) {
    if (complete) {
        return {
            content: null,
...
async function createReactionRoles(interaction) {
...
export async function confirmRoleAssignment(guildId, messageId, language, roleId) {
    await upsertRoleAssignment(messageId, guildId, language, roleId);
    await refreshReactionRoleMessage(guildId, messageId);
}
async function upsertRoleAssignment(messageId, guildId, language, roleId) {
...
async function addLanguage(interaction) {
    const guild = interaction.guild;
    if (!guild)
        return;
    const messageId = interaction.options.getString("message-id", true);
...
async function removeLanguage(interaction) {
    const guild = interaction.guild;
    if (!guild)
        return;
    const messageId = interaction.options.getString("message-id", true);
...
async function deleteReactionRoles(interaction) {
    const guild = interaction.guild;
    if (!guild)
        return;
    const messageId = interaction.options.getString("message-id", true);
...
export async function refreshReactionRoleMessage(guildId, messageId, client) {
    const record = await prisma.reactionRoleMessage.findUnique({
        where: { messageId },
        include: { roles: true }
    });
    ...
    if (!channel?.isTextBased())
        return;
    const message = await channel.messages.fetch(messageId).catch(() => null);
    await message?.edit(reactionRoleMessagePayload(record.languages, complete)).catch(() => undefined);
}
export async function refreshAllReactionRoleMessages(client) {
