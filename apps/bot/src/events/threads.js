...
export function registerThreadEvents(client) {
...
async function handleThreadCreate(thread) {
    if (!thread.guild || !thread.parentId || thread.type === ChannelType.PrivateThread || thread.parent?.nsfw)
        return;
...
async function handleThreadUpdate(thread) {
    if (!thread.guild)
        return;
...
async function backfillInitialThreadMessages(thread) {
    await delay(1000);
    ...
    for (const message of candidates) {
        if (message.author.bot || message.webhookId)
            continue;
        await translateMessage(message, { bypassCooldown: true });
    }
}
