...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const pairsInput = new TextInputBuilder()
      ...
      .setPlaceholder('123456789012345678:en\n987654321098765432:ru\n876543210987654321:de')
      .setRequired(true);

    modal.addComponents(new ActionRowBuilder<TextInputBuilder>().addComponents(pairsInput));
    await interaction.showModal(modal);

    const submission = await interaction
      .awaitModalSubmit({
        time: 5 * 60_000,
        filter: (i) => i.customId === 'link-channels-modal' && i.user.id === interaction.user.id,
      })
      .catch(() => null);
    ...
    for (const line of lines) {
      ...
      if (!isLanguageSupported(lang)) {
        errors.push(`${channelId}:${lang} — "${lang}" is not a supported language code`);
        continue;
      }
    ...
    const takenIds = new Set(existingMembers.map((m) => m.channelId));
